...
export const ClassesView = ({ api = classroomApi }) => {
  ...
  const openClass = async id => {
    ...
    setDetail(data); setReport(results); setMode('detail');
  };
    ...
    {loading ? <p role="status" className="p-6 text-slate-500">{say(lang, 'Loading classes…', '正在加载班级…')}</p> : <>
      {mode === 'home' && <>
        <section className={card}>
          {teacher ? <form onSubmit={e => { e.preventDefault(); run(async () => { const data = await api('/classes', { name: className }); setClassName(''); await openClass(data.class.id); }); }} className="space-y-4">
            ...
            <p className="text-sm text-slate-500">{say(lang, 'When you join, this teacher can see your name, submitted assignment answers and scores, and practice completion counts.', '加入后，本班老师可以查看你的姓名、已提交的作业答案和成绩，以及练习完成数量。')}</p>
