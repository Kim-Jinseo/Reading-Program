...
export function ProductiveAssignment({ data, lang, api, onBack }) {
  ...
  return <div className="space-y-6">
    ...
    {result ? <AssignmentFeedback assignment={assignment} attempts={attempts} lang={lang} onRetry={retry} /> : <section className={card + ' space-y-6'}>
      ...
      </> : <LessonSpeaking key={attempts.length} sentence={assignment.speaking?.sentence} hintZh={assignment.speaking?.hintZh} lang={lang} disabled={busy || Boolean(pending.current)} onRecording={setRecording} onCaptureStatus={setMicBusy} />}
