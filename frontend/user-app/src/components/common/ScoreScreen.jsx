...
export const ScoreScreen = ({ stars, onRetry, onContinue, customMessage }) => {
  ...
  return (
    <div className="bg-white p-6 sm:p-10 rounded-[2rem] shadow-lg border border-slate-100 text-center max-w-md mx-auto animate-in zoom-in-95 mt-6 sm:mt-10">
      <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-800 mb-6">{t('score_title')}</h2>
      
      <div className="flex justify-center gap-3 sm:gap-5 mb-4 items-center" style={{ minHeight: '72px' }}>
        {[1, 2, 3].map((starNum) => {
          ...
          return (
            ...
            >
              <Star size={size} className={`${className} w-11 h-11 sm:w-16 sm:h-16`} />
            </div>
          );
        })}
      </div>
      ...
      {(() => {
        ...
        return (
          <div className={`inline-flex max-w-full items-center justify-center gap-2 border-2 px-3 sm:px-6 py-2 rounded-full font-black text-base sm:text-lg leading-tight shadow-md mb-6 transition-all duration-500 ${
            isPurplePhase 
